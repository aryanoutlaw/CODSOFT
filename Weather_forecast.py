...
while True:
    ...
    if event == 'Submit':
        ...
        if weather in ('Partly Sunny','Partly Cloudy'):
            window['-IMAGE-'].update('symbols/partly-cloudy.png')
        
        # cloudy
        if weather in ('Mostly Cloudy','Cloudy','Overcast'):
            window['-IMAGE-'].update('symbols/cloud.png')
        # rain
        if weather in ('Rain','Chance of Rain','Light Rain','Showers','Scattered Showers','Rain and Snow','Hail','Heavy Rain','Light Rain Showers'):
